...
from app.models import User, Ninja, Team, Village
...
def find_village(db: Session, village_id: int, for_update: bool = False):
    q = db.query(Village).filter(Village.id == village_id)
    if for_update:
        q = q.with_for_update()
    village = q.first()
    if not village:
