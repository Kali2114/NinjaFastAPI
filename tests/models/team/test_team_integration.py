...
class TestTeamActionEndpointsIntegration:
    ...
    def test_create_team_without_kate(self, client_authed, db_session, setup_user):
        session = db_session
        create_team(session=session, user_id=setup_user.id)
        session.close()
    ...
    def test_patch_team_forbidden(self, client_authed, db_session):
        ...
        res = client_authed.put(f"/team/{ti.id}")
        assert res.status_code == 403
